...
export function SchoolComboBox() {
  ...
  const [schoolName, setSchoolName] = React.useState("Choose a school of choice")
  ...
  return (
    <>
      ...
      variant="outline" onClick={openDialog}>{payload.length === 0 ? <Loader2 className="mr-2 h-4 w-4 animate-spin" /> : schoolName} <School size={15}/></Badge>
        ...
        <CommandList>
          ...
          {payload.map((school) => (
            // to be set in contextAPI storage not session
              <CommandItem key={school.id}>
                <span className="flex uppercase" onClick={()=>{
                  sessionStorage.setItem("sid",school.id)
                  setSchoolName(school.name)
                  setOpen(!open)
                  }}><SchoolIcon /> {school.name}</span>
              </CommandItem>
            ))}
          </CommandGroup>
        </CommandList>
